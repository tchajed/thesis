...
good performance? This section evaluates performance both with a single and
multiple clients.
...
lose recently written data if the system crashes; we present some results
to show the benefit of providing this weaker guarantee.
...
other configurations with reduced concurrency. The ``seq wal'' configuration
...
\begin{figure}[ht]
  \begin{subfigure}[b]{0.5\textwidth}
    \centering
  \includegraphics[width=\textwidth]{daisy-nfs/fig/scale.pdf}
  \caption{On an NVMe disk}
  \label{fig:eval:scale:nvme}
  \end{subfigure}
  \begin{subfigure}[b]{0.5\textwidth}
    \centering
  \includegraphics[width=\textwidth]{daisy-nfs/fig/scale-ram.pdf}
  ...
  {Combined throughput of the \cc{smallfile} microbenchmark, varying the number
    ...
    cores similar to ext4, but both eventually scale sub-linearly. Linux
    is disk bandwidth-limited in the NVMe experiment, while DaisyNFS is limited by lock
    contention before hitting this I/O bottleneck.
    \tej{add lines for DaisyNFS with seq txn/wal directly to this graph}}
...
clients. Each data point is the result of averaging 5 separate runs, each for 10 seconds.
...
scalability in \cref{fig:eval:scale:nvme} is limited by disk throughput. DaisyNFS has a common
...
a time, performance improves with multiple clients since the work outside of the
file system can be parallelized across multiple cores. This work includes
the NFS client processing, sending and receiving packets on the local loopback
device, and sending and parsing the NFS RPCs.
