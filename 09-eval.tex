...
\begin{figure}[ht]
  ...
  {Combined throughput of the \cc{smallfile} microbenchmark, varying the number
    ...
    contention before hitting this I/O bottleneck. \tej{move in-memory graph to
      a separate figure, at end of section}}
...
clients. Each data point is the result of averaging 5 separate runs, each for 10
seconds. The results are shown in \cref{fig:eval:scale}. The graph shows that
...
\tej{explain the DFSCQ implemented the optimization, it is not out-of-reach for
  verification, but it breaks the GoTxn abstraction}
