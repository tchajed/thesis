...
DaisyNFS has a novel file-system design that relies on a \emph{transaction
system} to make verification practical. The advantage of this design is that the
...
implementation. While other file systems (including the most common Linux file
...
a formal specification for how the transaction system guarantees atomicity. Our
formal specification is backed by a machine-checked proof against an
implementation, giving confidence that this specification is actually achieved.
The challenge in formalizing the details is that the transaction system requires
calling code to follow some rules to guarantee atomicity --- for example,
transactions that read or write state unknown to the transaction system cannot
be made atomic.
...
and crash safety patterns. This thesis develops specialized infrastructure for
carrying out this reasoning: \textbf{Perennial} is a new program logic for
