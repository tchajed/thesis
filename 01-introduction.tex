...
non-determinism from crashes in the middle of execution and
exploiting concurrency in the implementation for good performance.
...
another verified, concurrent file system, Flashix. The use case for Flashix is
...
system which uses a disk that provides an abstraction of an array of blocks and
efficiently implements even write-write concurrency. In addition to different
goals for the overall artifacts, DaisyNFS acheives crash safety with a
general-purpose transaction system, whereas Flashix uses an ordered cache that
is carefully designed to work with its particular file-system design.
