...
\subsection[Specification for Append and TrimTill]{Specifications for \cc{Append} and \cc{TrimTill}}
...
$a \mapsto_{d}^{\mathrm{lftd}} o \sep \textlog{token}(a)$.} We can give
GoJournal's \cc{Commit} operation a
...
In the non-error, non-crash return, the postcondition turns $a \mapsto_{op} o'$
into a durable fact $a \mapsto_{d} o'$. If the system doesn't crash but the
transaction fails (which happens if it is too large to fit in the circular
buffer), then the caller gets back an unlifted disk fact $a \mapsto_{d} o$,
