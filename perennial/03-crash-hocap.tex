...
is not the circular buffer's concern.
...
func RecoverCircular() (c *Appender,
...
Definition append (s:circ_state) (new_upds: list update) :=
  s <[ upds := s.(upds) ++ new_upds ]>.

(* requires s.(start) <= new_start *)
Definition trim_till (s:circ_state) (new_start: u64) :=
  let num_removed := new_start - s.(start) in
  s <[ start := new_start ]>
    <[ upds := drop num_removed s.(upds) ]>.
\end{minted}
  \caption{The API for the circular buffer. Also included are the Coq
    definitions of the abstract transition system of the circular buffer.}
\end{figure}
...
state, noted as $\sigma \lappend \cc{upds}$ as shorthand for $\sigma$ with \cc{upds}
...
reasoning in the \ruleref{wpc-inv-alloc} rule in \cref{sec:perennial:wpc};
...
detail in \cref{s:design}. Exchanging gives threads a way to ``trade''
