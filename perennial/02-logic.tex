...
\indent \cc{Definition inv := "HA" :: P * "HB" :: Q * "HC" :: R},\\
combining the logical statement with the appropriate names. The notation
\cc{"HC" :: R} means the exact same thing as \cc{R}, logically, but we extend
the proof mode with the ability to destruct a hypothesis and automically name
its conjuncts. This has two advantages: first, we don't repeat the names for the
conjuncts throughout the proof, and second, extensions to the invariant are
minimally invasive to the proof (even if conjuncts are reordered) since each
name always refers to the same thing it did before.
...
\paragraph{Framing:} Recall that we mentioned the \ruleref{wpc-frame} rule for
proving a $\wpcw$ using a $\wpw$ when no durable resources are required for a
sub-part of the proof. This is commonly used, so we introduce a \cc{wpc_frame}
...
% tool for \sys even though the framework is implemented foundationally within
% Coq.
