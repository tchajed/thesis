...
We gave an intuitive meaning for $P \proves \wpr{e}{e_{r}}{Q}{Q_{c}}$. The
soundness theorem formally connects the definition of $\wprw$ to a guarantee
about $e$ and $e_{r}$ indendently of the details of the logic. The meaning of
the mechanisms of the logic is complicated due to features like ghost state,
invariants, and the purely-logical notion of ownership. The soundness theorem
establishes what an end-to-end theorem about the system \emph{means} without
trusting the interpretation and implementation of these features.
...
\label{thm:soundness}
  Let $d \in \textdom{Disk}$ be a disk. Let $X$ and $X_r$ be predicates over
  values.
  % \ralf{``pure'' is unspecified... maybe it is easier to use two sets of values instead? that makes ``pure'' implicit and hopefully intuitive.}
  Suppose that
  \[\bigast\limits_{a \mapsto v \in d} a \dmapsto v \proves
  \wpr{e}{e_r}{\Ret\var. X(\var)}{\Ret\varB. X_{r}(\varB)} \] is derivable in
  Perennial. If running $e$ (recovering with $e_{r}$) in a state $\sigma$ with an
empty heap and disk $d$ executes to an expression $e'$ in state $\sigma'$ (possibly
...
facts for all of the addresses in the initial disk. The first two conclusions
state that the normal postcondition and recovery postcondition have the intended
meaning for the return value of the main thread $e$ or the recovery procedure
$e_{r}$, respectively. Finally, the third conclusion is a \emph{safety}
statement about other threads in the system, which says that no forked threads
will get stuck.
...
For a system like a server, it is possible to go beyond this soundness theorem
and use Perennial to prove a property about its whole execution rather than the
final value (which is generally uninteresting for the main loop of a server).
The proof of a weakest precondition also maintains the invariants at each step,
and a stronger adequacy theorem states how these invariants relate to the
execution. We used this more general support to prove a \emph{refinement}
theorem for GoTxn that relates a whole execution of GoTxn's implementation to a
simpler specification execution.
...
\tej{say a lot more about how the intuition above relates to the real
implementation (laters, namespaces, masks)}
