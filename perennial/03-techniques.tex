...
complicated. The \ruleref{wpc-inv-alloc} rule gives the essence of the approach.

The specification for recovery is:

\newcommand{\circstate}{\cc{circular_state}}

\begin{align*}
  &\{ \circstate(\gamma, \sigma) \sep \cc{circ_resources}(\gamma) \} \\
  &\qquad\cc{recoverCircular()} \\
  &\{ \Ret{(\ell, \cc{diskStart}, \cc{upds})} \circstate(\gamma, \sigma) \sep {}  \\
  &\quad \sigma = (\cc{diskStart}, \cc{upds}) \sep {} \\
