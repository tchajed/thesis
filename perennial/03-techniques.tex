...
\subsection{Non-crash specifications for Append and TrimTill}
...
The circular buffer's proof also supplies this \cc{crash_obligation_alt}
theorem: \tej{come up with a name for crash obligation alt}
...
latter is ``immutable'' and held inside an invariant. \tej{examples of a P rec
  and a P crash? this is trivial in the circular buffer} Second, the theorem
...
In general, each layer supplies a crash obligation alt theorem. All of the layers' crash
...
the exchanger. The purpose of this predicate is to give the caller a way to, on
crash, relate resources issued by the library before a crash to those after a
crash. In the circular buffer, the $\startIs(\cc{start})$ and
$\diskendIs(\cc{end})$ resources are thrown away on crash and re-created during
recovery; there are only two of them, so it easy enough to reconstruct them
globally. In contrast, some resources are used locally by a thread and we would
like a way to retain ownership across a crash, into that thread's crash
condition. This is exactly what happens with GoJournal's lifting-based
specification, explained in greater detail in \autoref{s:design}. Exchanging
gives threads a way to ``trade'' ownership prior to a crash, associated with the
old $\gamma$ instance, for ownership after a crash, associated with $\gamma'$;
this ownership can also be exclusive, because the process destroys the old
ownership and thus can only be performed once.
...
for $a \mapsto_{op} o \sep a \mapsto_{d/2} o$; this gives the third type of
...
operation that modifies a single address, when in reality its main purpose is to
operate on multiple addresses; this is enough to understand the proof
issues involved. If the system doesn't crash, it is relatively straightforward
to reverse the lifting process and restore full ownership of the disk values.
$a \mapsto_{d} o'$ reflects that the commit actually succeeded in changing the
disk value, while $a \mapsto_{d} o$ results from aborting and throwing away
buffered writes in $op$.
...
system, the circular buffer, successfully wrote a single header block! \tej{need
