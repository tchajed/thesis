...
The write-ahead logging layer conveys durability by issuing a resource that
gives the history of multiwrites, then providing the caller an exchange lemma
that uses the WAL exchanger predicate to trade this assertion before a crash for
a similar assertion after a crash, albeit with some recent writes lost.
Crucially the effect of a crash is constrained by a durable lower bound in the
WAL's state. Because this is a lower bound even across a crash, the caller can
exchange persistent knowledge of the lower bound across a crash.
