...
the exchanger, one of the conclusions of the crash obligation alt theorem. The
...
\begin{itemize}
  ...
  \item A recovery theorem, with a crash condition, that re-builds the in-memory
        state of the system.
  \item A crash obligation alt theorem, to be called after the whole system has
        recovered, which sets up the system's invariant, the next generation,
        and gives an exchanger resource to relate the old state to the new one.
\end{itemize}

When layers are composed, the upper layer will generally subsume the recovery
and crash obligation alt theorems in its own proof.
