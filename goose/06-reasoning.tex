...
list. These theorems all require $i < \len(s)$.

The first is $\sliceFull(s, l) \vdash \sliceFull(s[:i], \take(l, i))$, capturing
how we can have full ownership over $s[:i]$ (including its capacity) by dropping
the rest of $s$.
...
\end{align*}
In just the left-to-right direction, this says that if we don't care about
ownership over capacity (and thus the right to append to any of these slices),
then we can split $s$ into two parts at the $i$th index.
...
no impact). As a corollary, we can split full ownership of a slice into two
asymmetric parts:
\begin{align*}
...
its corollary). It was interesting that we discovered these principles while
proving theorems in GooseLang. Even being experienced Go developers, we had
never thought about subslicing and appending in enough detail to appreciate this
difference in ownership reasoning.
