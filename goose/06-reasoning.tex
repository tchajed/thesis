...
As is typical in Goose, locks (Go's \cc{(*sync.Mutex)} type) are not built-in to GooseLang but modeled
...
signalled, but before the waiting thread wakes up other threads can wake up and
...
simply that \cc{Wait()} release and then acquires the lock associated with the
condition variable. \cc{Signal()} in reality affects the scheduling of
\cc{Wait()} calls. Go condition variables actually do provide a stronger
guarantee than in some other systems that \cc{Wait()} will only return if
