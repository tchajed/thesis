...
Map iteration in Go happens in a non-deterministic order (in fact the runtime
randomizes the iteration order to avoid accidentally relying on any particular
behavior). Thus strictly speaking $\cc{MapIter}$ should shuffle the elements of
the map before iterating over it, in order to model the non-determinism of the
