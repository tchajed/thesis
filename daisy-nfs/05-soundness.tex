...
stated as a theorem about an arbitrary program $p$ using the specification-level
API. To model that program using the transactions given by
implementation $i$, we write $\mathrm{link}(p, \atomiccomp i)$. The function
$(\atomiccomp i)(\opv) = \atomically{i(\opv)}$ gives an atomic transaction in the
GoTxn layer that implements $\opv$. Physically this
\cc{atomically} block corresponds to running code in a pattern like the
following, written using Go's support for generics:
%
\begin{minted}{go}
