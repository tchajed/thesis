DaisyNFS is the first verified file system that has both concurrency and crash
...
%% structures, and loop invariants, which DaisyNFS uses.
...
%% proof. DaisyNFS is of comparable complexity in 2,500 lines of imperative
...
%\paragraph{GoJournal} DaisyNFS's transaction system uses
...
%transactions do not access the same data. Instead of doing this manually, DaisyNFS extends GoJournal with
...
%allows DaisyNFS to use Dafny for sequential reasoning and benefit from
%proof automation.
...
%% DaisyNFS has a similar idea in proving that code running on the transaction system
%% behaves as if the transactions execute atomically. However, instead of using movers,
%% DaisyNFS uses Perennial's concurrent separation logic to show that the transaction system's
...
% the behavior of the lower-level handlers when run together; DaisyNFS combines a
...
% only used to verify a transaction system on paper.  DaisyNFS uses
...
% that handles a single request from the network or storage system. In DaisyNFS in
