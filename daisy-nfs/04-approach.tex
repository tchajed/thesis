...
abstraction of the following pseudo-code that looks more like the essence of the
...
Notice that this code starts with some recovery, restoring the transaction
system on line 3 and the file system on line 4. Then it repeatedly accepts new
requests from the network, abstracted with \cc{GetRequest()}. These requests are
each processed in a background thread due to the goroutine in line 7. The
processing for each request dispatches to the appropriate file-system operation
(e.g., lines 10 and 13). The implementations of these operations are compiled from Dafny
to Go, and then linked with the transaction system.

\tej{next, explain how $\sdfy$ fits in between these two abstractions and how
they relate via linking}

\resume

\tej{starting here the text is out-of-place}
