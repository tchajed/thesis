...
the default \cc{data=ordered} mode Linux achieves better performance but can
...
\begin{figure}
  ...
  \caption{Performance of Linux NFS and DaisyNFS for \cc{smallfile},
    \cc{largefile}, and \cc{app} workloads. DaisyNFS achieves comparable
    performance to ext4 in \cc{data=journal} mode.}
  \label{fig:eval:bench}
\end{figure}

The results are shown in \autoref{fig:eval:bench}. DaisyNFS gets about the same
...
\cc{data=ordered} mode.

DaisyNFS achieves good performance on the app workload, which consists of running
\cc{git clone} on the xv6 repo followed by \cc{make}. xv6 is an operating
...
\begin{figure}
  ...
  \caption{Combined throughput of the \cc{smallfile} microbenchmark running on
    an SSD while
...
the Coq proof.
% Finally, we trust our execution model: the user needs to run
% initialization from an empty disk, run recovery after each boot, and the disk
% should preserve all data written.

\subsection{Testing the specification}
...
%%   \autoref{sec:dafny:freeing} through the whole stack afterward. (3 days, 1400
%%   lines changed)
%% \end{itemize}
