...
The correctness for GoTxn relates a \emph{spec program}
...
transaction $\atomically{\cc{f}}$ with a sequence \cc{tx := Begin(); f(tx); tx.Commit()}
(with some additional code handling aborts).

\tej{refinement could be its own small subsection before this, and this just
defines GooseLang layers}
