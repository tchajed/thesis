...
a specification for the WAL's operations that is consumed by the rest of the
proof concerning the code using the write-ahead log. It is not materialized at
...
The specification for \cc{ReadMem(a)} if the address \cc{a} is not in the log says that it
