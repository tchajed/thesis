...
\tej{Joe points out that this is a jarring transition --- split into one section on verifying
GoTxn's atomicity (lifting + transaction refinement) and another with proof
internals (WAL + logical atomicity + obj).}
...
this code uses a
pair of conversion functions \cc{byte_to_bits} and \cc{bit_to_bytes} that go
between an 8-bit integer (a \cc{u8}) and \cc{list bool} (of length 8). To reason
